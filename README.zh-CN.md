...
- 可选接入 IsThereAnyDeal，补充跨店当前最优价、价格历史、Steam 商店史低、商店列表和价格历史查询。
...
| [`app APPID`](#app) | 综合游戏信息、价格、折扣结束时间、评价、标签、Deck、分级、链接、购买选项、媒体、新闻；`--enhanced` 可补充高级价格增强信息 | [`appdetails`][source-appdetails]、[`IStoreBrowseService/GetItems`][source-storebrowse]、[`appreviews`][source-appreviews]、[`GetNumberOfCurrentPlayers`][source-currentplayers]、[`GetNewsForApp`][source-news]、[高级价格映射][source-itad-lookup] |
| [`price APPID`](#price) | 价格、折扣、区域价格、折扣结束时间；`--compare` 可对比多个地区；`--enhanced` 可补充高级价格增强信息 | [`appdetails`][source-appdetails]、[`IStoreBrowseService/GetItems`][source-storebrowse]、[高级价格映射][source-itad-lookup] |
...
| [`history APPID`](#history) | 游戏的 Steam 价格历史，并可选扩展到授权商店 | [高级价格历史][source-itad-history] |
...
`--enhanced` 会在默认 Steam 数据之外额外补充可选的高级价格信息，目前包括：

- 跨店当前最优 deal
- 历史最佳 deal
- 多时间窗口历史低价
- 第三方价格后端视角下的 Steam 商店史低
- 价格页面链接和当前 deal 链接

当前版本里，`--enhanced` 还不能和 `price --compare` 同时使用。
...
`history` 保持 Steam 优先：默认只显示该游戏在 Steam 商店的价格变化。加上 `--all-stores` 时，才会扩展到价格后端支持的授权非 Steam 商店。

如果你想直接看“历史折扣窗口”而不是原始价格变动点，可以使用 `--sales`。这个视图会根据价格变动日志推断每一段折扣开始于何时、结束于下一次价格变动何时。

在 `--json` 模式下，原始价格历史和推断出的折扣区间都会额外附带 RFC3339 精确时间与 Unix 时间戳，方便脚本处理。
...
活动数据来自 Steamworks 官方 Upcoming Steam Events 页面，以及公开 Steam 商店特卖页面（例如农历新年页面），会解析活动名称、日期范围、状态、类型、PT 时区标记、简介/资格说明、注册链接和详情链接。当商店特卖页暴露内嵌事件元数据时，JSON 还会返回精确 Unix `start_time`/`end_time`、`store_url`、事件/组 id、公告链接，以及 `image_url`、`title_image_url`、`capsule_image_url`、`background_image_url` 等图片资源。使用 `--store-sales=false` 可只显示 Steamworks 活动日历。
默认使用较宽的时间窗口（`--past-days 365 --future-days 365`），让年度特卖页面在结束后仍然可见；只想看近期活动时再用 flags 缩小范围。

终端表只展示 6 列(Start、End、Status、Category、Event、Description)。`--json` 会返回完整字段，包括 `registration_url`、`info_url`、`store_url`、`notes`、精确时间和图片 URL。
...
| [`https://store.steampowered.com/sale/{vanity}`][source-store-sale-pages] | Steam 商店公开特卖页面，包括第三方和地区特卖活动 |
...
| [`https://api.isthereanydeal.com/games/lookup/v1`][source-itad-lookup] | 把 Steam appid 映射到高级价格后端的游戏 id |
| [`https://api.isthereanydeal.com/games/overview/v2`][source-itad-overview] | 最优 deal 快照、bundle 引用、跨店价格对比 |
| [`https://api.isthereanydeal.com/games/history/v2`][source-itad-history] | 高级价格变化历史日志 |
| [`https://api.isthereanydeal.com/games/historylow/v1`][source-itad-historylow] | 多时间窗口历史低价快照 |
| [`https://api.isthereanydeal.com/games/storelow/v2`][source-itad-storelow] | 单个商店历史低价，例如 Steam 商店史低 |
| [`https://api.isthereanydeal.com/service/shops/v1`][source-itad-shops] | 高级价格后端内部使用的商店映射 |
