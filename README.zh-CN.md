...
```text
--cc        国家/地区价格区，默认 CN。运行 `steam-cli locales --type regions` 查看常用值。
--lang      Steam 语言，默认 schinese。运行 `steam-cli locales --type languages` 查看可选值。
...
| [`https://steamcommunity.com/id/{vanity}/?xml=1`][source-community-id-xml] | 公开 profile XML |
...
- profile URL / vanity / SteamID64 解析。
