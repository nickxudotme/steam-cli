...
活动数据来自 Steamworks 官方 Upcoming Steam Events 页面，以及公开 Steam 商店特卖页面（例如农历新年页面），会解析活动名称、日期范围、状态、类型、PT 时区标记、简介/资格说明、注册链接和详情链接。使用 `--store-sales=false` 可只显示 Steamworks 活动日历。
